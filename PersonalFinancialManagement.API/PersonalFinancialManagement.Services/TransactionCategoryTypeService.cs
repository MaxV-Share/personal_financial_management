    ...
    ITransactionCategoryTypeService
{
        ...
        TransactionCategoryTypeCreateRequest? request)
    {
        ...
        async void Action()
        {
            ...
            var countAffect = await _unitOffWork.Repository<TransactionCategoryType, Guid>()
                .CreateAsync(entity);
    ...
    {
        var transactionCategoryType = await _unitOffWork.Repository<TransactionCategoryType, Guid>()
            .GetByIdNoTrackingAsync(id);
        var result = _mapper.Map<TransactionCategoryTypeViewModel>(transactionCategoryType);
        return result;
    }

    public async Task<IBasePaging<TransactionCategoryTypeViewModel>?> GetPagingAsync(
        IFilterBodyRequest request)
    {
        var query = _mapper.ProjectTo<TransactionCategoryTypeViewModel>(_unitOffWork
            .Repository<TransactionCategoryType, Guid>().GetNoTrackingEntities());
